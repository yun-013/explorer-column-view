...
- **接続した機器・ネットワークドライブの表示** — ホーム列の顔ぶれをエクスプローラーの「PC」に揃えます。`DriveInfo.GetDrives()` に載らないものを `IShellItem` でシェル名前空間から補って表示
  - **ポータブルデバイス（MTP/PTP）** — iPhone・Android・デジタルカメラなど。ドライブ文字を持たないため `DriveInfo` では見えません。総容量／空き容量のゲージも表示（機器直下のストレージから取得）
    - 実体がファイルシステム上に無いため、カラムには展開せず `Enter`／ダブルクリックでエクスプローラーに引き渡します（右クリックはシェル本来のメニューがそのまま出るので「取り出し」等も可能）
  - **切断中のネットワークドライブ（NAS 等）** — マッピングは記憶されているが未接続の状態。ドライブ文字自体が存在しないため `GetLogicalDrives()` にも載りません
    - **自動で繋ぎ直します** — 起動時と、ネットワーク復帰時（`NetworkAddressChanged`）に、記憶されたマッピング（`HKCU\Network\<文字>`）のうち未接続のものを保存済みの資格情報で接続。Windows のログオン時再接続が失敗して「切断中」で残った NAS も、アプリを開けば繋がります
    - 接続は裏で走るのでホーム列の表示は待たされません（先に「切断中」の行として出し、繋がってから容量ゲージ付きの通常のドライブに差し替え）。繋がらない相手には 2 分の間隔を空けて再試行し、資格情報ダイアログは勝手には出しません
    - 自動で繋がらなかった場合は `Enter`／ダブルクリックで接続（このときだけ Windows 自身の資格情報ダイアログが出ます）。選択しただけでは接続を試みないので、`↑↓` で行を通り過ぎても待たされません
  - **抜き差しに自動追従** — `WM_DEVICECHANGE`（ボリューム＋WPD デバイスインターフェイス）を購読し、接続・取り外しでホーム列を自動で読み直します。USB メモリなどドライブ文字の付く機器も同様
- **タブ** — `Ctrl+T` 新規タブ / `Ctrl+W` タブを閉じる / `Ctrl+Shift+T` 閉じたタブを開き直す（列の並びと選択・元の位置まで閉じたときのまま復元。消えたフォルダから右は落ちる。履歴 10 件・全ウィンドウ共有なので、タブ 1 枚のウィンドウを閉じた場合も別のウィンドウで戻せる）
- **ウィンドウ** — `Ctrl+N` で現在のフォルダを新しいウィンドウで開く。タスクバーのアイコンを右クリックしたメニュー（ジャンプリスト）の「新しいウィンドウ」からも開ける（起動済みなら同じプロセスがウィンドウを増やす）
...
| `Ctrl+N` | 新しいウィンドウ（現在のフォルダ） |
| `Ctrl+Shift+T` | 閉じたタブを開き直す |
| `Ctrl+H` | 隠しファイルの表示切替 |
| `Ctrl+L` | パスバーへフォーカス |
| `Alt+←` / `Alt+→` / `Alt+↑` | 戻る / 進む / 上へ |
| `F5` | 表示中のタブを再読み込み（検索結果は同じ検索語で再検索） |
